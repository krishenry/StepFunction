...
StepARN=$(aws stepfunctions list-state-machines --region $REGION --query 'stateMachines[?name==`Kris-StepFunction`].{stateMachineArn:stateMachineArn}' ) # | jq -r '.stateMachines' | jq -r '.stateMachineArn')
